...
public class MailConfiguration {
  ...
  public JavaMailSender javaMailSender() {
    ...
    if (enabled
      ...
      System.err.println(
          "[MAIL WARNING] Mail is enabled but username/password are missing. Configure MAIL_USER"
              + " and MAIL_PASSWORD (Google App Password recommended).");
    }
    if (enabled && starttls && port == 465) {
      System.err.println(
          "[MAIL INFO] You are using port 465 (implicit SSL). Consider setting"
              + " starttls.enable=false and using 'mail.smtp.ssl.enable=true' if needed, or switch"
              + " to port 587 for STARTTLS.");
    }
    return mailSender;
  }
