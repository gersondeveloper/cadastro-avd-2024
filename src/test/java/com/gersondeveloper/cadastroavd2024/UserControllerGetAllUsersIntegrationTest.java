...
class UserControllerGetAllUsersIntegrationTest extends AbstractIntegrationTest {
  ...
  @Test
  @DisplayName(
      "GET /api/user/all?role=ADMIN&sortBy=name&direction=DESC returns all users sorted by name"
          + " DESC")
  ...
  void getAllUsers_asUser_returnsCustomers() throws Exception {
    var c1 =
        new UserResponse(
            ...
            LocalDateTime.now(),
            true);
    Mockito.when(userService.findAllByRole(Mockito.eq(UserRole.CUSTOMER), any()))
  ...
  void getAllUsers_asCustomer_returnsEmpty() throws Exception {
    mockMvc
        .perform(
            get("/api/user/all")
                .param("role", UserRole.CUSTOMER.name())
                .header("Api-Version", "v1")
                .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
